...
Если запустить два потока одновременно, то порядок их запуска определяет ОС. Это зависит от загрузки системы, приоритетов и т.д.
...
С каждым объектом ассоциирована специфичная структура внутри JVM - монитор. Когда поток входит в synchronized секцию, у монитора прописывается владелец. 
Это называется захватом монитора. Захват происходит на уровне JVM и недоступен программисту напрямую.
